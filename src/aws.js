...
function buildUserDataScript(githubRegistrationToken, label, noRunner = false) {
    ...
    if (config.input.ec2Os === 'windows') {
      if (config.input.eniId) {
        return [
          ...
          `$result = aws ec2 attach-network-interface --network-interface-id ${config.input.eniId} --instance-id $instanceId --network-card-index 1 --device-index 1 | ConvertFrom-Json`,
  ...
  if (config.input.ec2Os === 'windows') {
    // We cannot attach ENI first because it will break 169.254.169.254 access
    if (config.input.eniId) {
  ...
  } else if (config.input.ec2Os === 'linuxRocky8') {
    if (config.input.runnerHomeDir) {
      // If runner home directory is specified, we expect the actions-runner software (and dependencies)
      // to be pre-installed in the AMI, so we simply cd into that directory and then start the runner
      return [
...
async function startEc2Instance(label, githubRegistrationToken) {
  ...
  if (config.input.label) {
    userData = buildUserDataScript(githubRegistrationToken, label, true);
  } else {
